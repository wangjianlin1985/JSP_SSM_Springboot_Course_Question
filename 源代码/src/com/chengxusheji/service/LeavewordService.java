...
public class LeavewordService {
    ...
    /*按照查询条件分页查询留言记录*/
    public ArrayList<Leaveword> queryLeaveword(String leaveTitle,UserInfo userObj,String leaveTime,String teacherNo,int currentPage) throws Exception { 
    	...
    	if(!teacherNo.equals("")) where = where + " and t_leaveword.teacherNo like '%" + teacherNo + "%'";
    	int startIndex = (currentPage-1) * this.rows;
    	return leavewordMapper.queryLeaveword(where, startIndex, this.rows);
    }

    /*按照查询条件查询所有记录*/
    public ArrayList<Leaveword> queryLeaveword(String leaveTitle,UserInfo userObj,String leaveTime,String teacherNo) throws Exception  { 
     	String where = "where 1=1";
    	if(!leaveTitle.equals("")) where = where + " and t_leaveword.leaveTitle like '%" + leaveTitle + "%'";
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_leaveword.userObj='" + userObj.getUser_name() + "'";
    	if(!leaveTime.equals("")) where = where + " and t_leaveword.leaveTime like '%" + leaveTime + "%'";
    ...
    /*当前查询条件下计算总的页数和记录数*/
    public void queryTotalPageAndRecordNumber(String leaveTitle,UserInfo userObj,String leaveTime,String teacherNo) throws Exception {
     	String where = "where 1=1";
    	if(!leaveTitle.equals("")) where = where + " and t_leaveword.leaveTitle like '%" + leaveTitle + "%'";
    	if(null != userObj &&  userObj.getUser_name() != null && !userObj.getUser_name().equals(""))  where += " and t_leaveword.userObj='" + userObj.getUser_name() + "'";
    	if(!leaveTime.equals("")) where = where + " and t_leaveword.leaveTime like '%" + leaveTime + "%'";
