...
public class LeavewordController extends BaseController {
	...
	public void list(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String teacherNo,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		...
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, teacherNo, page);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, teacherNo);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String teacherNo,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		if (currentPage==null || currentPage == 0) currentPage = 1;
		if (leaveTitle == null) leaveTitle = "";
		if (leaveTime == null) leaveTime = "";
		if (teacherNo == null) teacherNo = "";
		List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle, userObj, leaveTime, teacherNo, currentPage);
	    /*计算总的页数和总的记录数*/
	    leavewordService.queryTotalPageAndRecordNumber(leaveTitle, userObj, leaveTime, teacherNo);
	    /*获取到总的页码数目*/
	    int totalPage = leavewordService.getTotalPage();
	...
	public void OutToExcel(String leaveTitle,@ModelAttribute("userObj") UserInfo userObj,String leaveTime,String teacherNo, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(leaveTitle == null) leaveTitle = "";
        if(leaveTime == null) leaveTime = "";
        if(teacherNo == null) teacherNo = "";
        List<Leaveword> leavewordList = leavewordService.queryLeaveword(leaveTitle,userObj,leaveTime,teacherNo);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "Leaveword信息记录"; 
        String[] headers = { "留言id","留言标题","留言人","留言时间","老师解答","解答老师","解答时间"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<leavewordList.size();i++) {
        	Leaveword leaveword = leavewordList.get(i); 
        	dataset.add(new String[]{leaveword.getLeaveWordId() + "",leaveword.getLeaveTitle(),leaveword.getUserObj().getName(),leaveword.getLeaveTime(),leaveword.getReplyContent(),leaveword.getTeacherNo(),leaveword.getReplyTime()});
        }
        /*
