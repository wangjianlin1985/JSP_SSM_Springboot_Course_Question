...
public class ClassInfoController extends BaseController {
	...
	public void list(String classNo,String colleageName,String className,String bornDate,String mainTeacher,Integer page,Integer rows, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
		if (page==null || page == 0) page = 1;
		if (classNo == null) classNo = "";
		if (colleageName == null) colleageName = "";
		if (className == null) className = "";
		if (bornDate == null) bornDate = "";
		if (mainTeacher == null) mainTeacher = "";
		if(rows != 0)classInfoService.setRows(rows);
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo, colleageName, className, bornDate, mainTeacher, page);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(classNo, colleageName, className, bornDate, mainTeacher);
	    /*获取到总的页码数目*/
	...
	public String frontlist(String classNo,String colleageName,String className,String bornDate,String mainTeacher,Integer currentPage, Model model, HttpServletRequest request) throws Exception  {
		...
		if (bornDate == null) bornDate = "";
		if (mainTeacher == null) mainTeacher = "";
		List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo, colleageName, className, bornDate, mainTeacher, currentPage);
	    /*计算总的页数和总的记录数*/
	    classInfoService.queryTotalPageAndRecordNumber(classNo, colleageName, className, bornDate, mainTeacher);
	    /*获取到总的页码数目*/
	    int totalPage = classInfoService.getTotalPage();
	    ...
	    request.setAttribute("colleageName", colleageName);
	...
	public void OutToExcel(String classNo,String colleageName,String className,String bornDate,String mainTeacher, Model model, HttpServletRequest request,HttpServletResponse response) throws Exception {
        if(classNo == null) classNo = "";
        if(colleageName == null) colleageName = "";
        if(className == null) className = "";
        if(bornDate == null) bornDate = "";
        if(mainTeacher == null) mainTeacher = "";
        List<ClassInfo> classInfoList = classInfoService.queryClassInfo(classNo,colleageName,className,bornDate,mainTeacher);
        ExportExcelUtil ex = new ExportExcelUtil();
        String _title = "ClassInfo信息记录"; 
        String[] headers = { "班级编号","所在学院","班级名称","成立日期","班主任"};
        List<String[]> dataset = new ArrayList<String[]>(); 
        for(int i=0;i<classInfoList.size();i++) {
        	ClassInfo classInfo = classInfoList.get(i); 
        	dataset.add(new String[]{classInfo.getClassNo(),classInfo.getColleageName(),classInfo.getClassName(),classInfo.getBornDate(),classInfo.getMainTeacher()});
        }
        /*
